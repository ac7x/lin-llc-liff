                        ...
                        return (
                            ...
                            >
                                <div className="flex flex-col gap-1 mb-2">
                                    <span className="font-semibold text-base">{epic.title}</span>
                                    <span className="text-xs text-gray-500">
                                        （{progress.percent}％）
                                    </span>
                                ...
                                <div className="grid grid-cols-2 gap-x-2 gap-y-1 text-sm">
                                    ...
                                    <span className="text-gray-500">保險狀態：</span>
                                    <span>{epic.insuranceStatus || "無"}</span>
                                    <span className="text-gray-500">負責人：</span>
                                    <span>{epic.owner?.name || "未指定"}</span>
                                    <span className="text-gray-500">監工：</span>
                                    <span>
                                        {epic.siteSupervisors
                                            ?.map((s) => s.name)
                                            .join("、") || "-"}
                                    </span>
                                    <span className="text-gray-500">安全衛生人員：</span>
                                    <span>
                                        {epic.safetyOfficers
                                            ?.map((s) => s.name)
                                            .join("、") || "-"}
                                    ...
                                    <span className="text-gray-500">工作區：</span>
                                    <span>
                                        {Array.isArray(epic.workZones) &&
                                            epic.workZones.length > 0
                                            ? epic.workZones.map((z) => z.title).join(", ")
