  ...
  return (
    <>
      <main className="p-4 max-w-screen-lg mx-auto">
        ...
        <div className="mb-4">
          <select className="border p-2 rounded" value={selectedEpicId} onChange={e => setSelectedEpicId(e.target.value)}>
        ...
        <div className="md:hidden flex flex-col gap-4">
          {[...filteredTasks]
            ...
            .map(task => {
              ...
              return (
                <div key={task.taskId} className="rounded-lg border border-gray-300 bg-white shadow-sm p-3">
                  ...
                  {relatedLoads.length > 0 && (
                    <div className="overflow-x-auto">
                      <table className="table-auto w-full border-collapse border border-gray-200 text-xs">
                        <thead>
                          <tr>
                            <th className="border px-1 py-0.5">工作量名稱</th>
                            <th className="border px-1 py-0.5">計畫數量</th>
                            <th className="border px-1 py-0.5">單位</th>
                            <th className="border px-1 py-0.5">計畫開始</th>
                            <th className="border px-1 py-0.5">計畫結束</th>
                            <th className="border px-1 py-0.5">完成數量</th>
                            <th className="border px-1 py-0.5">執行者</th>
                            <th className="border px-1 py-0.5">備註</th>
                          </tr>
                        </thead>
                        <tbody>
                          {relatedLoads.map(load => (
                            <tr key={load.loadId}>
                              <td className="border px-1 py-0.5">{load.title || load.loadId}</td>
                              <td className="border px-1 py-0.5">{load.plannedQuantity}</td>
                              <td className="border px-1 py-0.5">{load.unit}</td>
                              <td className="border px-1 py-0.5">{load.plannedStartTime?.slice(0, 10) || ""}</td>
                              <td className="border px-1 py-0.5">{load.plannedEndTime?.slice(0, 10) || ""}</td>
                              <td className="border px-1 py-0.5">{load.actualQuantity}</td>
                              <td className="border px-1 py-0.5">{Array.isArray(load.executor) ? load.executor.join("、") : ""}</td>
                              <td className="border px-1 py-0.5">{load.notes || ""}</td>
                            </tr>
                          ))}
                        </tbody>
                      </table>
