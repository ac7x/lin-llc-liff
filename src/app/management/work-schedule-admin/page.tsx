...
import Timeline, { Id, TimelineGroupBase, TimelineItemBase } from 'react-calendar-timeline';
import { getAllWorkEpics, updateWorkEpicWorkLoads } from './work-schedule-admin.action';
...
const parseEpicSnapshot = (
	...
	const epics: WorkEpicEntity[] = docs.map((doc) => ({
		...doc,
		epicId: doc.epicId,
	}));
...
const WorkScheduleAdminPage: React.FC = () => {
	...
	const groups: TimelineGroupBase<Id>[] = useMemo(() => {
	...
	const items: TimelineItemBase<Id>[] = useMemo(
		() =>
			epics.flatMap((e) =>
				(e.workLoads || [])
					.filter((l) => l.plannedStartTime && l.plannedStartTime !== '')
					.map((l) => {
						...
						const end = l.plannedEndTime && l.plannedEndTime !== '' ? parseISO(l.plannedEndTime) : addDays(start, 1);
						return {
							id: l.loadId,
							group: e.epicId,
			...
			<div className="flex-none h-[60vh] w-full flex items-center justify-center relative">
				...
				>
					<Timeline
						...
						onItemResize={(itemId, time, edge) => void handleItemResize(itemId as string, time, edge)}
						onItemDoubleClick={handleItemRemove}
						groupRenderer={({ group }) => (
							<div className="px-2 py-1 text-neutral-900 dark:text-neutral-100">{group.title}</div>
						)}
						itemRenderer={({ item, getItemProps, getResizeProps }) => {
							const { left: leftResizeProps, right: rightResizeProps } = getResizeProps();
							const dateStr = `${format(item.start_time, 'yyyy/MM/dd (EEE) HH:mm', {
								locale: zhTW,
			...
			>
				<div className="w-full h-full flex flex-col p-4 mx-auto">
					...
					{unplanned.length === 0 ? (
						...
						<div
							className="flex flex-nowrap gap-3 overflow-x-auto pb-16 px-0.5 w-full"
							tabIndex={0}
