...
const WorkScheduleAdminPage: React.FC = () => {
	...
	const [{ isOver }, drop] = useDrop({
		accept: 'unplanned',
		drop: (item: LooseWorkLoad, monitor) => {
			const offset = monitor.getClientOffset()
			if (!offset) return
			const timeline = document.querySelector('.rct-scroll')
			if (!timeline) return
			const timelineRect = timeline.getBoundingClientRect()
			const dragTime = new Date(timelineRect.left + offset.x).getTime()
			const newGroupOrder = Math.floor((offset.y - timelineRect.top) / 50)
			handleItemMove(item.loadId, dragTime, newGroupOrder)
		},
		collect: monitor => ({
			...
			<DndProvider backend={HTML5Backend}>
				<div className="min-h-screen w-full bg-black dark:bg-neutral-900 flex flex-col">
					...
					<div className="flex-none h-[60vh] w-full flex items-center justify-center relative">
						<div ref={drop} className="w-full h-full rounded-2xl bg-white dark:bg-neutral-800 border border-gray-300 dark:border-neutral-700 shadow overflow-hidden" style={{ minWidth: '100vw', height: 400 }}>
							<Timeline
								groups={groups}
								items={items}
						...
						<div className="w-full h-full flex flex-col p-4 mx-auto">
							...
							{unplanned.length === 0 ? (
								...
								<div className="flex flex-nowrap gap-3 overflow-x-auto pb-16 px-0.5 w-full" tabIndex={0} aria-label="unplanned-jobs">
									{unplanned.map(wl => (
										<UnplannedWorkItem key={wl.loadId} workLoad={wl} />
									))}
								</div>
							)}
						</div>
...
const UnplannedWorkItem: React.FC<{ workLoad: LooseWorkLoad }> = ({ workLoad }) => {
	const [, drag] = useDrag({
		type: 'unplanned',
		item: workLoad
	})
